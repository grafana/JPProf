 ...
 * <p>
 * Protobuf definition:
 * https://github.com/google/pprof/blob/44fc4e887b6b0cfb196973bcdb1fab95f0b3a75b/proto/profile.proto
 */
public class jfr2pprof {
    ...
    private static byte[] getMethodName(final JfrReader reader, final long methodId, final byte methodType) {
        ...
        if ((methodType >= FlameGraph.FRAME_NATIVE && methodType <= FlameGraph.FRAME_KERNEL) || className == null
                || className.length == 0) {
            // Native method
            return methodName;
        } else {
